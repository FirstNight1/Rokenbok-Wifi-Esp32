"""
Simplified OTA (Over-The-Air) update page for RokVision web interface

Provides web UI for:
- Uploading complete RokVision folders
...
def handle_post(body, cfg):
    ...
    try:
        ...
        if action == "upload_folder":
            return handle_folder_upload(fields, body, cfg)
        elif action == "github_preview":
            return handle_github_preview_post(fields, cfg)
        elif action == "github_download":
            return handle_github_download(fields, cfg)
        elif action == "restart":
            return handle_restart(cfg)
        else:
...
def handle_github_preview_post(fields, cfg):
    ...
    folder = fields.get("folder", "RokVision").strip()
